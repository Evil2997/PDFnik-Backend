...
def create_pdf(
    ...
    left_margin = 50
    top_margin = height - 50  # координата верхнего поля для текста
    bottom_margin = 50
    line_height = 16

    # фактический размер верхнего поля (в пунктах) — от края страницы
    top_margin_size = height - top_margin
    ...
    if not has_content:
        c.drawString(left_margin, top_margin, "Пустой PDF (нет текста и изображений)")

    c.save()


# =========================
#   ЛОГИКА РЕНДЕРА КАРТИНОК
# =========================
...
def _maybe_resample_to_dpi(
    ...
    Возвращает:
        (новый_img, новый_width_px, новый_height_px, draw_w, draw_h)

    ВАЖНО: draw_w/draw_h не меняем — физический размер на странице остаётся тем же.
    """
    if width_px <= 0 or height_px <= 0:
        return img, width_px, height_px, draw_w, draw_h

    # сколько пикселей реально нужно для заданного размера на странице
    ...
    # если и так не больше цели — не трогаем
