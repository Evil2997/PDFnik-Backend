...
def create_pdf(
    ...
    try:
        if text:
            logger.info("create_pdf: drawing text block")
        ...
        if image_paths:
            logger.info(f"create_pdf: drawing {len(image_paths)} images")
        ...
        if not has_content:
            logger.info("create_pdf: no content, writing placeholder text")
            c.drawString(
                layout.left_margin,
