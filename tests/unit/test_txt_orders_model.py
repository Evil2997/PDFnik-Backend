"""
Тесты для Pydantic-моделей в txt_orders.py.

Покрываем:
- normalize_legacy_payload: старый формат полей → новый
- TxtTranscribeJob валидация нового формата
- TxtDoneResult
- edge cases: отсутствующие поля, None-значения
"""
import pytest
...
class TestNormalizeLegacyPayload:
    """
    normalize_legacy_payload должен принимать старый формат сообщений
    (плоские поля chat_id, storage_key, input_url) и превращать их
    в новый вложенный формат (target, reply, delivery).
    """
