"""
Tests for run_logic.py.

Coverage:
- make_run_key / txt_name_for_run — determinism and stability
- run_once — cache hit (OK), cache miss (failed), full run, transcription error
"""
...
class TestMakeRunKey:
    ...
    def test_different_models(self):
        assert make_run_key("id", _cfg(model="base"), "int8") != make_run_key(
            "id", _cfg(model="large"), "int8"
        )

    def test_different_beam(self):
        assert make_run_key("id", _cfg(beam_size=5), "int8") != make_run_key(
            "id", _cfg(beam_size=1), "int8"
        )
...
class TestRunOnceCacheHit:
    def test_cache_hit_ok_returns_cached(self, tmp_dir):
        """If the file exists and the status is "ok," return the cache; do not invoke the engine."""
        prepared = _prepared(tmp_dir)
        cfg = _cfg()
        engine = _mock_engine()

        # Create the result file in advance
        from main_app.domain.work_with_pdf.actions.files.run_logic import (
            make_run_key,
            resolve_compute_type,
            txt_name_for_run,
    ...
    def test_cache_hit_failed_reruns(self, tmp_dir):
        """
        CRITICAL BUG: If the previous run failed,
        the file may exist, but it cannot be cached.
        The engine must be invoked again.
        """
        prepared = _prepared(tmp_dir)
        cfg = _cfg()
        engine = _mock_engine()
        ...
        # Should have restarted the transcription
    ...
    def test_allow_skip_false_ignores_cache(self, tmp_dir):
        """`allow_skip=False` — ignore the cache, even if the status is `ok`."""
        prepared = _prepared(tmp_dir)
        cfg = _cfg()
        engine = _mock_engine()
...
class TestRunOnceFailure:
    ...
    def test_failed_result_not_cached_on_rerun(self, tmp_dir):
        """After a failed upsert, the next call should not retrieve from the cache."""
        prepared = _prepared(tmp_dir)
        cfg = _cfg()
        engine = MagicMock()
